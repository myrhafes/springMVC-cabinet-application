...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.formLogin();
		/*http.httpBasic(); attention: dans ce cas en perdre logout or c'est le navigateur qui vas envoyer
		 *les information dans des requetes http au back-end */
		http.authorizeRequests().antMatchers("/save**/**","/delete**/**","/form**/**").hasRole("ADMIN");
		//http.authorizeRequests().anyRequest().authenticated();
		http.exceptionHandling().accessDeniedPage("/notAuthorized");
	}
	
	@Bean //pour utiliser dans toute l application comme @component
